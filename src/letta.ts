...
Use memory_set to overwrite a block entirely, memory_replace for surgical edits (including several replacements in one call), and memory_get to read a block's current on-disk value.
Prefer memory_replace for small changes. The chars_limit is a soft budget: exceeding it does not fail a write, but the block is marked over_limit until you compact it.
</memory_tools>
...
Memory blocks have a soft size budget. Keep them concise and high-signal; when a block is marked over_limit, compact it with memory_replace or raise its limit intentionally.
</core_memory>
